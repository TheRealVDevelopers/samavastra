...
export const login =
  ({ loginData }) =>
    async (dispatch) => {
      ...
      // Auto-register first admin account if not found
      if (!data.success && (data.message.includes('auth/user-not-found') || data.message.includes('auth/invalid-credential'))) {
        const regData = await authService.register({ registerData: { ...loginData, role: 'CEO', department: 'Management' } });
        if (regData.success) {
          data = await authService.login({ loginData }); // Login again to fetch profile
        }
      }
